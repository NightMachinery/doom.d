...
*** [help:night/mistral-fim-insert-at-point] used to say nothing at all. It now reports =FIM: requesting …= with a =⋯= indicator at the insertion point, then the outcome with its elapsed time.
...
*** [help:night/mistral-fim-get] now takes =:on-error= and =:finally= alongside =:callback=, mirroring plz's own then/else split. [help:night/h-mistral-fim--api-message] reads Mistral's =detail= as well as =message=, so a bad key reads =FIM: HTTP 401 — Invalid API Key=.
*** [help:night/h-mistral-fim--cancel] drops the buffer-local pending slot *before* killing curl. Killing it makes plz report a curl failure, and that handler must already look stale or every cancellation would announce itself as an error.
*** So a second =M-.= supersedes the first request rather than racing it, and =C-g= aborts. [help:night/h-mistral-fim-escape] returns nil, because =doom-escape-hook= runs under =run-hook-with-args-until-success= and a non-nil return would swallow the quit.
