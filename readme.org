...
*** Four commands send buffer text to a third party: [help:night/fim-insert-at-point] on =M-.=, and [help:night/ellama-code-fill-in-the-middle] on =leader . .=, [help:night/ellama-code-complete], [help:night/ellama-complete]. Until now the path policy covered exactly one of them, so =leader . .= would happily stream a decrypted =.gpg= buffer that =M-.= refuses outright -- and at ten times the window, +-10000 chars.
...
*** The shared machinery is in =autoload/night-llm-context.el= rather than either caller: =night-mistral-fim.el= is wrapped in =(after! (night-openai night/ellama) ...)= and provides no feature, so its body evaluates strictly after =night-ellama.el='s in the same provide cascade and nothing can depend on it. The old =night/fim-*= names keep obsolete aliases, declared *before* their referents so a customized value is not dropped.
