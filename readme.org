...
*** [help:night/hear] checks too, so something lands in the echo area; [help:night/audio-path-checkable-p] decides what Emacs may check, excluding URLs and zsh named dirs such as =~mu/=.
*** See [[file:docs/audiofile-mpv-server.md][docs/audiofile-mpv-server.md]].
