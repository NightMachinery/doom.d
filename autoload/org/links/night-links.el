...
;;;
  ...
  (dolist (type (list "user" "username" "pass" "password" "ip" "rss" "studentID" "nationalID" "mailto" "postal" "postalCode" "github" "dead" "bale"))
...
;;;
  ...
  (comment
   ;; @upstreamRegression [jalali:1401/09/07] [help:org-toggle-link-display] seems to need to be called in org buffers now, so we can't just call it here. Added to [help:night/org-interactive-startup] instead.
   )
  ;; Update [jalali:1404/06/30/22:09]: reverted this and disabled the line in night/org-interactive-startup
           ...
           (parent-2
            (or (ignore-errors ;; with-demoted-errors ;; ignore-errors
                  (--> bfn
                       ...
                       (funcall preserve it "meetings")
                       (funcall preserve it "seminars")
                       (funcall preserve it "online")
                       (funcall preserve it "code")
                       (funcall preserve it "rules")
                       (prog1 (f-dirname it)
                         (message "it: %s" it))
                       (f-base it)
...
;;;
  (defun night/org-rm-link-to-desc-at-point ()
  ...
  (defun night/org-rm-link-to-desc (beg end)
    (interactive "r")
    (save-mark-and-excursion
      (save-restriction
        ...
        (while (re-search-forward org-link-bracket-re nil t)
          ...
          (night/org-rm-link-to-desc-at-point)))))
;;;
  )
